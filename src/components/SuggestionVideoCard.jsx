      ...
      <Link to={`/video/${video.videoId}`}>
        <div className="flex mb-3">
          ...
          <div className="flex flex-col ml-3 overflow-hidden">
              <span className='text-sm lg:text-xl xl:text-sm font-bold line-clamp-2 text-white' title={video?.title}>
                {video?.title}
              </span>
              <span className='text-[12px] lg:text[10px] xl:text-[12px] font-semibold mt-2 text-white/[0.7] flex items-center' title={video?.author?.title}>
                {video?.author?.title}
